...
async function loadDropdowns() {
    try {
        const res = await fetch('../src/backend.php?action=get_dropdowns');
        const data = await res.json();
...
async function loadTowns(countryId) {
    ...
    try {
        const res = await fetch(`../src/backend.php?action=get_towns&countryId=${countryId}`);
...
async function loadAllData() {
    ...
    try {
        // Load patients first
        const patientsRes = await fetch('../src/backend.php?action=get_patients');
        ...
        // Try to load medications, but don't fail if it errors
        try {
            const medicationsRes = await fetch('../src/backend.php?action=get_medications');
...
window.deletePatient = async function(id) {
    ...
    try {
        ...
        const res = await fetch('../src/backend.php', { method: 'POST', body: formData });
        const data = await res.json();
...
window.editMedication = async function(id) {
    try {
        const res = await fetch(`../src/backend.php?action=get_medication&id=${id}`);
        const data = await res.json();
        if (!data.success) throw new Error(data.message);
